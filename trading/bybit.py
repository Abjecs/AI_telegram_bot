...
class BybitClient:
    ...
    async def _request(self, method, path, params=None, body=None, private=False):
        ...
        if private:
            ...
            signature = hmac.new(BYBIT_API_SECRET.encode(), raw.encode(), hashlib.sha256).hexdigest()
            headers.update({
        ...
        async with self.session.request(
            ...
            data=payload if method == "POST" else None, headers=headers,
        ) as response:
            ...
            try:
                ...
                raise BybitError(f"Bybit returned non-JSON response HTTP {response.status}: {preview}") from exc
    ...
    async def funding(self, symbol):
        return (await self._request("GET", "/v5/market/funding/history", {
            "category": "linear", "symbol": symbol, "limit": 1,
        })).get("list", [{}])[0]

    async def instrument(self, symbol):
        return (await self._request("GET", "/v5/market/instruments-info", {
            "category": "linear", "symbol": symbol,
        }))["list"][0]
    ...
    async def amend_order(self, symbol, order_id=None, order_link_id=None, price=None, qty=None):
        return await self._request("POST", "/v5/order/amend", body={
            "category": "linear", "symbol": symbol,
            "orderId": order_id, "orderLinkId": order_link_id,
            "price": price, "qty": qty,
        }, private=True)
    ...
    async def close_market(self, symbol, position):
        ...
        return await self._request("POST", "/v5/order/create", body={
            "category": "linear", "symbol": symbol, "side": side,
            "orderType": "Market", "qty": position["size"],
